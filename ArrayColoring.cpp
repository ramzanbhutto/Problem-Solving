...
void solve(){
  ...
  vb visited(n, false);
  for(int i=0; i<n; i++){
    if(i&1) visited[i]=true;  // true(red color) for odd indices and false(blue color) for even indices
  }

  umi pos_map;
  for(int i=0; i<n; i++){
    pos_map[nums[i]]=i;      // mapping each number to its index in the original array
  }
  
  sort(all(nums));
  for(int i=0; i<n-1; i++){
    int idx1= pos_map[nums[i]];  // getting the original index of current number in the sorted array
    int idx2= pos_map[nums[i+1]];  // same scene as above for the next number in the sorted array
    if(visited[idx1] == visited[idx2]){  // if adjacent numbers in sorted array have same color, then it's not possible to color the array as required
      cout<<"NO\n";
      return;
    }
  }

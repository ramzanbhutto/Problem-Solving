...
void solve(){
  ...
  ll min_buses= (tires+5)/6;   // ceil(ties/6) as if n%6!=0 then we need one extra bus to cover leftover wheels and same scence for 4
  ll max_buses= tires/4;
  if(min_buses>max_buses) cout<<-1<<'\n';
  else cout<<min_buses<<" "<<max_buses<<'\n';
}

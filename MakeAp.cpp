...
void solve(){
  ...
  // modifying b 
  // as b has to divided by 2, so first find sum and check that sum is even or not
  ll sum= a+c;
  if(sum%2==0){
    ll _b= sum/2;
    if(_b>0 && _b%b==0){
  ...
  cout<<"NO\n";  // if none of a, b, c can be modified to make AP then it is impossible to make AP
}
